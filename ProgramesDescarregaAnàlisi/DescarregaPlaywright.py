...
def main(excel_path: str):
    estat_path = "estat.txt"
    if os.path.exists(estat_path):
        with open(estat_path) as f:
            ultim_codi = f.read().strip()
    else:
        ultim_codi = None

    saltar = True if ultim_codi else False
    ...
    with sync_playwright() as p:
        ...
        for _, row in df.iterrows():
            ...
            if saltar:
                if code == ultim_codi:
                    saltar = False
                    continue  # NO processis el codi que hi ha a estat.txt!
                else:
                    continue  # Salta fins trobar el codi
